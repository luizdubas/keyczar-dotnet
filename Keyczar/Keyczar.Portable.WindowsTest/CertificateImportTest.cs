...
namespace KeyczarTest
{
    ...
    public class CertificateImportTest : AssertionHelper
    {
        ...
        [Test]
        public async Task TestCryptImport([Values("rsa")] String keyType, [Values("pem", "der")] String fileFormat)
        {
            using (
                var keyset = await ImportedKeySet.Import.X509Certificate(KeyPurpose.Encrypt,
        ...
        [Test]
        public async Task TestSignImport([Values("rsa", "dsa")] String keyType, [Values("pem", "der")] String fileFormat)
        {
            using (var signer = new Signer(Util.TestDataPath(TEST_DATA, keyType + "-sign")))
            {
                var signature = signer.Sign(input);
                using (
                    var keyset = await ImportedKeySet.Import.X509Certificate(KeyPurpose.Verify,
                                                                       Util.TestDataPath(TEST_DATA,
                                                                                         keyType + "-sign-crt." +
                                                                                         fileFormat)))
