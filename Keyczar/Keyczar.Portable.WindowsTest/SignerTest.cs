...
namespace KeyczarTest
{
    ...
    public class SignerTest : AssertionHelper
    {
        ...
        [TestCase("hmac")]
        public async Task TestSignerVerify(String subDir)
        {
        ...
        public async Task TestPublicVerify(String subDir, string nestDir)
        {
        ...
        public async Task TestPublicVerifySizes(String subDir, string nestDir)
        {
            var subPath = Util.TestDataPath(TEST_DATA, subDir, nestDir);
            ...
            using (var publicVerifier = new Verifier(subPath + ".public"))
            {
                ...
                {
                    var activeSignature =
                        (WebBase64) await Util.ReadFirstLine(Path.Combine(subPath, String.Format("{0}.out", size)));
        ...
        [TestCase("rsa-sign", "")]
        [TestCase("rsa-sign", "unofficial")]
        public async Task TestBadVerify(String subDir, string nestDir)
        {
            var subPath = Util.TestDataPath(TEST_DATA, subDir, nestDir);
            using (var verifier = new Signer(subPath))
            {
