...
namespace KeyczarTest
{
    ...
    public class PkcsImportTest : AssertionHelper
    {
        ...
        private async Task<Stream> HelperOpenPkcsStream(String keyType, String fileFormat, String keyPurpose)
        {
            return await ApplicationData.Current.LocalFolder.OpenStreamForReadAsync(Util.TestDataPath(TEST_DATA, keyType + "-" + keyPurpose + "-pkcs8." + fileFormat));
        }


        [Test]
        public async Task TestCryptImport(
            [Values("rsa")] string keyType,
            [Values("pem", "der")] string format)
        {
            using (var keystream = await HelperOpenPkcsStream(keyType, format, "crypt"))
            using (
        ...
        [Test]
        public async Task TestCryptImportBadKey(
            [Values("dsa")] string keyType,
            [Values("pem", "der")] string format)
        {
            using (var keystream = await HelperOpenPkcsStream(keyType, format, "sign"))
            {
                Expect(
                    () =>
        ...
        [Test]
        public async Task TestSignImport(
