...
namespace Keyczar.Compat
{
    ...
    public class ImportedKeySet : IKeySet, IDisposable
    {
        ...
        public class Importer
        {
            ...
            /// <returns></returns>
            public async Task<ImportedKeySet> PkcsKey(KeyPurpose purpose, string path, Func<string> passwordPrompt = null)
            {
            ...
            /// <exception cref="InvalidKeySetException">DSA key cannot be used for encryption and decryption!</exception>
            /// <exception cref="InvalidKeySetException">Unsupported key type!</exception>
            public async Task<ImportedKeySet> X509Certificate(KeyPurpose purpose, string path)
            {
                using (var stream = await ApplicationData.Current.LocalFolder.OpenStreamForReadAsync(path).ConfigureAwait(false))
