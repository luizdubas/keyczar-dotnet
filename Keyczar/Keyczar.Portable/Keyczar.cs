...
namespace Keyczar
{
    ...
    public abstract class Keyczar:IDisposable
    {
        ...
        private readonly Dictionary<int, List<KeyValuePair<KeyVersion, Key>>> _hashedKeys;
        ...
        private static Dictionary<int, List<KeyValuePair<KeyVersion, Key>>> HashKeys(IList<Tuple<byte[],KeyVersion,Key>> versions)
        {
            return versions
                .Select(t => new {Hash = t.Item1, Version = t.Item2, CryptKey = t.Item3})
                .ToLookup(k => Utility.ToInt32(k.Hash), v => v)
                .ToDictionary(k => k.Key,
                              v => new List<KeyValuePair<KeyVersion, Key>> (v.ToDictionary(vk => vk.Version, vv => vv.CryptKey)));
        ...
        protected IEnumerable<Key> GetKey(byte[] hash)
        {
            ...
            List<KeyValuePair<KeyVersion, Key>> list;
